...
def model():
    ...
    model=LogisticRegression(solver=solve.get(),penalty=penn.get())
...
def files():
    with open(r"C:\Users\musan\Desktop\model summary","w",encoding="utf-8") as file:
        file.write("You have use Logistic Regression model \n")
        file.write("\n")
        file.write(f"The columns used for features are {feature_col} and the targetted columns are {target_col}\n")
        file.write("\n")
        file.write(f"The Hyper parameters used in the model are initiated as Solver - {solve.get()} , Penalty - {penn.get()}\n")
        file.write("\n")
...
Label(root,font="System",text="solver").place(x=20,y=330)
Label(root,font="System",text="penalty").place(x=20,y=360)


split = StringVar()
choose = ttk.Combobox(root, width = 22, textvariable = split)
choose['values'] = ('0.2','0.25','0.3')
choose.place(x=220,y=300)

solve = tk.StringVar()
choose = ttk.Combobox(root, width = 22, textvariable = solve)
choose['values'] = ('liblinear','lbfgs','newton-cg','sag','saga')
choose.place(x=220,y=330)

penn = tk.StringVar()
choose = ttk.Combobox(root, width = 22, textvariable = penn)
choose['values'] = ('l1','l2','elasticnet')
choose.place(x=220,y=360)
